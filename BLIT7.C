...
int colours[20] = {2, 1, 3, 4, 5, 6, 7, 8, 1, 10,
                  11, 12, 13, 14, 15, 4, 2, 3, 9, 5};

int vertices[12][3] = {{P, 0, U}, {P, 0, -U}, {-P, 0, -U}, {-P, 0, U},
                       {U, P, 0}, {-U, P, 0}, {-U, -P, 0}, {U, -P, 0},
                       {0, U, P}, {0, -U, P}, {0, -U, -P}, {0, U, -P}};
int faces[20][3] = {{0, 1, 7}, {0, 4, 1}, {6, 2, 3}, {2, 5, 3},
                    {7, 6, 9}, {6, 7, 10}, {4, 5, 11}, {4, 8, 5},
                    {9, 8, 0}, {8, 9, 3}, {10, 11, 2}, {10, 1, 11},
                    {0, 7, 9}, {7, 1, 10}, {9, 6, 3}, {6, 10, 2},
                    {1, 4, 11}, {0, 8, 4}, {5, 8, 3}, {5, 2, 11}};
 
extern void mode_ati();
extern void mode_8514();
extern void line_8514(int x0, int y0, int x1, int y1);
extern void fill_8514(int x0, int y0, int x1, int y1, int colour);
extern void blit_8514(int x0, int y0, int x1, int y1, int colour, int shiftx, int shifty);
extern void colour_blit_8514(int x0, int y0, int x1, int y1, int shiftx, int shifty);
extern void blank_8514(int x0, int y0, int x1, int y1);
extern void set_scissor_8514(int x0, int y0, int x1, int y1);
extern void xor_8514();
extern void noxor_8514();
extern void set_current_8514(int x0, int y0);
extern void set_fgcolour_8514(char colour);
...
int orthonormal(int x0, int y0, int z0,
            int x1, int y1, int z1)
{
   long t, s = 0;
   double d;
                    ...
                    int n)
{
   ...
   if ((col = orthonormal(x1 - x0, y1 - y0, z1 - z0, x2 - x0, y2 - y0, z2 - z0)) == 0)
      return 0;
...
int main(void)
{
  ...
  for (i = 0; i < 2560; i++)
  {
     ...
     noxor_8514();
     blank_8514(640, 0, 768, 128);

     for (n = 0; n < 20; n++)
     {
        if ((col = rotate_triangle(&rx0, &ry0, &rx1, &ry1, &rx2, &ry2,
                            xs, ys, zs, n)) > 0)
        {
           draw_triangle(col, rx0 + 64, ry0 + 64,
                         rx1 + 64, ry1 + 64, rx2 + 64, ry2 + 64);
        }
     }

     colour_blit_8514(640, 0, 768, 128, 640, 0);
  }
...
void draw_triangle(int col, int x0, int y0, int x1, int y1, int x2, int y2)
{
   ...
   blit_8514(rect_x0, rect_y0, rect_x1, rect_y1, col, 0, 240);
